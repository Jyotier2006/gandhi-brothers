...
export default function CheckoutPage() {
  ...
  const validateField = (name: keyof CheckoutAddress, value: string): string | undefined => {
    switch (name) {
      ...
      case "email":
        return /^\S+@\S+\.\S+$/.test(value) ? undefined : t("errEmail");
      case "phone":
        return /^[6-9]\d{9}$/.test(value) ? undefined : t("errPhone");
      case "address":
        return value.trim() ? undefined : t("errAddress");
      case "city":
        return value.trim() ? undefined : t("errCity");
      case "state":
        return value.trim() ? undefined : t("errState");
      case "pincode":
        return /^\d{6}$/.test(value) ? undefined : t("errPincode");
      default:
        return undefined;
    }
  };
  ...
  const placeOrder = async () => {
    ...
    setSubmitting(true);
    const loadingId = toast.loading(t("toastStarting"));
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || t("toastCouldNotStart"));

      if (typeof window === "undefined" || !window.Razorpay) {
        throw new Error(t("toastStillLoading"));
      }
        ...
        handler: async (response: {
          ...
          try {
            ...
            if (!verifyRes.ok || !verifyData.success) {
              throw new Error(verifyData.error || t("toastCannotConfirm"));
            }
            clearCart();
            localStorage.removeItem("gandhi-checkout");
            router.push(`/order-success?orderId=${encodeURIComponent(verifyData.orderId)}`);
          } catch (err) {
            const reason = err instanceof Error ? err.message : t("toastCannotConfirm");
      ...
      rzp.on("payment.failed", (resp: { error?: { description?: string } }) => {
        const reason = resp?.error?.description || t("toastDeclined");
        router.push(`/order-failed?reason=${encodeURIComponent(reason)}`);
      });
      ...
      toast.dismiss(loadingId);
      toast.error(err instanceof Error ? err.message : t("toastGeneric"));
      setSubmitting(false);
    }
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] text-terracotta relative overflow-hidden">
        <div className="relative z-10 flex flex-col items-center gap-4 animate-pulse">
          ...
          <p className="font-semibold tracking-widest text-[10px] uppercase">{t("initiating")}</p>
        </div>
      </div>
    );
  }
  ...
  if (items.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[75vh] max-w-lg mx-auto text-center px-4">
        ...
        <h1 className="text-4xl font-sans font-bold text-ink mb-4">{t("emptyTitle")}</h1>
        <p className="font-serif text-lg text-ink/70 mb-10 max-w-sm">
          {t("emptyBody")}
        </p>
        <Link href="/products">
          <Button size="lg" className="h-14 px-10 rounded-full shadow-lg">{t("browseProducts")}</Button>
        </Link>
      </div>
    );
  }
            ...
            <div className="lg:col-span-7 xl:col-span-8">
              <div className="bg-white/80 backdrop-blur-xl border border-white rounded-[2rem] p-6 sm:p-10 shadow-2xl shadow-ink/5">
                <div className="flex items-center gap-3 mb-8">
                  ...
                  <h2 className="text-2xl font-bold font-sans text-ink">{t("shippingDetails")}</h2>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <div className="sm:col-span-2 space-y-2">
                    <Label htmlFor="name" className="text-ink/80 font-semibold ml-1">{t("fullName")}</Label>
                    <Input
                      id="name" name="name"
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="email" className="text-ink/80 font-semibold ml-1">{t("email")}</Label>
                    <Input
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="phone" className="text-ink/80 font-semibold ml-1">{t("phone")}</Label>
                    <Input
                  ...
                  <div className="sm:col-span-2 space-y-2 pt-2">
                    <Label htmlFor="address" className="text-ink/80 font-semibold ml-1">{t("address")}</Label>
                    <Input
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="city" className="text-ink/80 font-semibold ml-1">{t("city")}</Label>
                    <Input
                      id="city" name="city"
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="state" className="text-ink/80 font-semibold ml-1">{t("state")}</Label>
              ...
              <div className="sticky top-24 bg-white border border-white rounded-[2rem] shadow-2xl shadow-ink/10 overflow-hidden">
                <div className="p-8">
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <h2 className="text-2xl font-bold font-sans text-ink">{t("payment")}</h2>
                  </div>
                  ...
                  <div className="space-y-3 text-[15px] font-medium text-ink/80 mb-6 border-b border-ink-100/50 pb-6">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      {shipping !== null ? (
                        ...
                        <span className="text-ink/40 text-sm italic">{t("enterPincode")}</span>
                      )}
                    </div>
                    <div className="flex justify-between items-center">
                  ...
                  <div className="flex justify-between items-end mb-8">
                    <span className="block text-sm text-ink/50 font-bold uppercase tracking-wider mb-1">{t("toPay")}</span>
                    {shipping !== null ? (
                      <span className="text-3xl font-extrabold text-terracotta tracking-tight">{formatINR(total)}</span>
                    ) : (
                      <span className="text-sm font-semibold text-ink/40 italic">{t("enterPincodeAbove")}</span>
                    )}
                  </div>
                  ...
                  >
                    {submitting ? (
                      <span className="animate-pulse">{t("loadingGateway")}</span>
                    ) : !shipping ? (
                      <>{t("enterPincodeToContinue")}</>
                    ) : (
                      <>{t("paySecurely", { total: formatINR(total) })}</>
                    )}
                  </Button>
                </div>
      ...
      {confirming && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-3xl shadow-2xl p-10 w-full max-w-sm text-center border border-ink-50">
            ...
            <h2 className="text-2xl font-bold font-sans text-ink mb-2">{t("confirmingTitle")}</h2>
